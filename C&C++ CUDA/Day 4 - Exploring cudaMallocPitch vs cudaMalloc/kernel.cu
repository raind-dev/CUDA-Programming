...
__global__ void traverse_device_data_address_by_pitch(int* device, size_t pitch, int width) {
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int* row = (int*)((char*)device + x * pitch);
    for (int i = 0; i < width; i++) {
        printf("thread idx: %d, address: %p\n", x, (void*)(row + i));
    }
}

__global__ void matrix_operations_V1(int* mat, int* mul, int* scl, int* res, int col, size_t mat_pitch, size_t mul_pitch, size_t res_pitch) {
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    for (int i = 0; i < col; i++) {
        int tmp = 0;
        for (int j = 0; j < col; j++) {
            tmp += mat[x * mat_pitch_idx + j] * mul[i + j * mul_pitch_idx];
        }
        tmp += scl[x * res_pitch_idx + i];
        res[x * res_pitch_idx + i] = tmp;
    }
}

__global__ void matrix_operations_V2(int* mat, int* mul, int* scl, int* res, int col, size_t mat_pitch, size_t mul_pitch, size_t res_pitch) {
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
	int mat_pitch_idx = mat_pitch / sizeof(int);
	int mul_pitch_idx = mul_pitch / sizeof(int);
...
int main()
{
    ...
    //traverse_device_data_address_by_pitch << <grid, block_v1 >> > (gpu_mat, mat_pitch, col);
    //cudaDeviceSynchronize();

    matrix_operations_V1 << < grid, block_v1 >> > (gpu_mat, gpu_mul, gpu_scl, gpu_res, col, mat_pitch, mul_pitch, res_pitch);
    cudaDeviceSynchronize();
    ...
    matrix_operations_V2 << < grid, block_v2 >> > (gpu_mat, gpu_mul, gpu_scl, gpu_res, col, mat_pitch, mul_pitch, res_pitch);
    cudaDeviceSynchronize();
