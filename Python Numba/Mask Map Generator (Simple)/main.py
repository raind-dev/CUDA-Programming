...
results = np.zeros_like(img_rgb)
print(f"Image Shape: {img_rgb.shape} | Image Type: {type(img_rgb)} | Image Data Type: {img_rgb.dtype}")

print("\n\n---------Initialize GPU Resources:")
threadsperblock = (16, 16) # according to image width and height
blockspergrid_x = (img_rgb.shape[0] + threadsperblock[0] - 1) // threadsperblock[0] # In case the image size is not an integer multiple of the block size, enough blocks are still allocated to process the entire image.
blockspergrid_y = (img_rgb.shape[1] + threadsperblock[1] - 1) // threadsperblock[1]
blockspergrid = (blockspergrid_x, blockspergrid_y)
print(f"Allocate threads: {threadsperblock} | Allocate blocks: {blockspergrid}")
print("Copy data to device: ")
d_img = cuda.to_device(img_rgb)
